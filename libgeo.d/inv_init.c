...
int (*inv_trans[])();	/* inverse function pointer			*/
#else
long insys;		/* input system code				*/
long inzone;		/* input zone number				*/
double *inparm;		/* input array of projection parameters		*/
long indatum;		/* input datum code				*/
char *fn27;		/* NAD 1927 parameter file			*/
char *fn83;		/* NAD 1983 parameter file			*/
...
long (*inv_trans64[MAXPROJ + 1])();
			/* 8 byte inverse function pointer array for 	*/
			/* Power Challenge				*/
...
-----------------------------------*/
  ...
  if (insys == UTM)
     ...
     *iflg64 = utminvint(r_major,r_minor,scale_factor,zone);
        *iflg = *iflg64;
     inv_trans64[insys] = utminv;
     #if defined(SGI64)
     inv_trans[insys] = (int (*)()) inv_trans64[insys];
     #else
     inv_trans[insys] = inv_trans64[insys];
     #endif
     }
  else
  if (insys == SPCS)
